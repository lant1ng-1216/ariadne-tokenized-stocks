...
**Figure 4. Capability evidence matrix.** Capability status is separated from the broader acceptance count so that an upstream blocker cannot be hidden by an aggregate score.

The current audit records 88 acceptance items: 80 verified and 8 incomplete or externally blocked. The source of truth is [`docs/PHASE_ACCEPTANCE.md`](PHASE_ACCEPTANCE.md), and the evidence inputs are maintained in [`research/data/`](../research/data/).
...
**Figure 5. API behavior and failure taxonomy.** HTTP status and business code are classified separately so that upstream failures, rate limits and successful responses remain distinguishable.
...
**Figure 6. Request observability trace.** Request duration, attempt number, status, business code, rate-limit headers and final classification form the evidence surface for diagnosis.
...
The figure generator reads only the JSON evidence files in `research/data/` and writes SVG files to `research/figures/generated/`. No credentials or wallet secrets are required to regenerate the static figures.
